...
export function realEstateTotals(
  ...
  const debt = loans.reduce((acc, l) => {
    const share = l.propertyId ? shareById.get(l.propertyId) : undefined;
...
/** Capital restant dû des prêts conso (sans bien rattaché), en EUR. */
export function consoDebtEur(loans: Loan[], rates: FxRates, today: string = todayKey()): number {
  return loans
    .filter((l) => !l.propertyId && today >= l.startDate)
    .reduce((acc, l) => acc + toEur(loanBalanceAt(l, today), l.currency, rates), 0);
}
 ...
 * `net` : déduit le capital restant dû des crédits (immobiliers et conso).
 * Les prêts conso sont déduits même si `properties` est vide (immobilier masqué).
 */
export function buildPatrimoineSeries(
  ...
  const consoAt = (day: string) => (net ? consoDebtEur(loans, rates, day) : 0);

  if (accountPoints.length >= 2) {
    return accountPoints.map((pt) => ({
      date: pt.date,
      value: pt.value + realEstateAt(activeProps, loans, series, rates, pt.date, net) - consoAt(pt.date),
    }));
  }
  ...
  let day = from;
  while (day <= today) {
    points.push({ date: day, value: realEstateAt(activeProps, loans, series, rates, day, net) - consoAt(day) });
    if (day === today) break;
    const next = addDays(day, step);
    day = next > today ? today : next;
  }
